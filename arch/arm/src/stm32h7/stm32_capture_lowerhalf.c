...
static int stm32_cap_handler(int irq, void * context, void * arg)
{
  ...
  //syslog(LOG_DEBUG, "CH: %d, period: %d, clock: %ld", ch, period, lower->clock);
  ...
  lower->freq = period; //We set presceler for timer period 1 sec
  ...
  switch (priv->channel)
    {
      case 1:
        ...
        STM32_CAP_SETCLOCK(priv->cap, priv->clock, 10000, maxtimeout);
